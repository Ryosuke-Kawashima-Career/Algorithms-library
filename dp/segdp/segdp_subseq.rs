...
// abc339E: segment tree DP
// segtreeで部分列の長さを格納する
// LISの値の差の絶対値がd以内バージョン
fn main() {
    input!{n: usize, d: i64, a: [i64; n]}
    // 部分列の最大の長さ = seg_dp[ai: last element(a[i])]
    let mut seg_dp = SegTree::new(&vec![0; MAX+1], max_func, -1);

    let mut max_length: i64 = 0;
    for i in 0..n {
        // [l r]
        let l: usize = 0.max(a[i] - d) as usize;
        let r: usize = (MAX as i64).min(a[i] + d) as usize;
        let length: i64 = seg_dp.prod(l, r+1) + 1;
        seg_dp.set(a[i] as usize, length);
