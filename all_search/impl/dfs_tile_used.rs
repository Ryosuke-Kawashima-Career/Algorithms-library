...
// abc345D: O(n!*2^n*HW)
// タイルを敷き詰める問題: 使うタイルをあらかじめbitで計算する
...
fn main() {
    ...
    let bits: usize = 1 << n;
    for bit in 0..bits {
        let used_tiles: Vec<usize> = (0..n).filter(|&tile| bit >> tile & 1 == 1).collect();
        // O(n!) -> O(n_used!)に改善する
        let n_used: usize = used_tiles.len();
        // タイルの置く順番をpermutationで決める
        for perm in (0..n_used).permutations(n_used) {
            if dfs(n_used, h, w, &tiles, &used_tiles, &perm, &mut graph) {
                is_ok = true;
            }
        }
    }
...
fn dfs(phase: usize, h: i64, w: i64, tiles: &Vec<(i64, i64)>, 
    used_tiles: &Vec<usize>, order: &Vec<usize>, graph: &mut BTreeSet<(i64, i64)>
) -> bool {
    ...
    let (height, width) = tiles[used_tiles[order[phase-1]]];
    ...
    // タイルを置かない
    if dfs(phase-1, h, w, tiles, used_tiles, order, graph) {
        is_possible = true;
    }
