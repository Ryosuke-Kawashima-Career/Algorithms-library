...
// abc147D
// Q. Σi=0..n Σj=i+1..=n (a[i] ^ a[j])
// 桁ごとにxor -> 桁ごとの寄与率を計算する
fn main() {
    input!{n: usize, a: [i64; n]}
    // ある桁において立っている個数 = pop_digits[桁]
    let mut pop_digits: Vec<i64> = vec![0; N];
    for digit in 0..N {
        for i in 0..n {
            pop_digits[digit] += (a[i] >> digit) & 1;
        }
    }
    ...
    for digit in 0..N {
        // 2^x * (0^1の組み合わせ)
        sum += ModInt(2).pow(digit as i64) * ModInt::new(pop_digits[digit] * (n as i64 - pop_digits[digit]));
    }
