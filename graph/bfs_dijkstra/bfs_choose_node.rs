...
fn main() {
    ...
    let is_reachable_to: Vec<bool> = reachable_from(0, &is_reachables);
    if is_reachable_to[n_potion-1] {
        println!("Yes");
    } else {
        println!("No");
    }
...
fn reachable_from(start: usize, is_reachables: &Vec<Vec<bool>>) -> Vec<bool> {
    ...
    while let Some(v) = que.pop_front() {
        for next in 0..n {
            if !seen[next] && is_reachables[v][next] {
